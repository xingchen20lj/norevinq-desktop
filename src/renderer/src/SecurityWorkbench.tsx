...
export function SecurityWorkbench({ snapshot, project, close, onError }: {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}>
    <section className="settings-workbench security-workbench" role="dialog" aria-modal="true" aria-label="Norevinq 安全工作台">
      <header className="settings-workbench-header">
        <div><p className="eyebrow">NOREVINQ SECURITY</p><h2>Norevinq 安全工作台</h2></div>
        <button className="icon-button" onClick={close} aria-label="关闭 Norevinq 安全工作台"><X size={16} /></button>
      </header>
      ...
      <div className="settings-workbench-body">
        {tab === 'overview' && <Overview snapshot={snapshot} findings={findings} busy={busy} refresh={() => run(() => window.norevinq.refreshSecurityRuntime())} />}
