  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}>
    <section className="settings-workbench" role="dialog" aria-modal="true" aria-label="设置工作台">
      <header className="settings-workbench-header">
        <div><p className="eyebrow">NOREVINQ SETTINGS</p><h2>设置与集成</h2></div>
...
function ApplicationTab({ snapshot, diagnostics, busy, run, onUpdate, onDiagnostics }: {
  ...
  return <div className="settings-section">
    <div className="section-heading"><div><h3>应用更新</h3><p>由签名发布包中的 electron-builder 更新元数据驱动。</p></div>{canCheck && <button disabled={busy} onClick={() => void run(async () => onUpdate(await window.norevinq.checkForUpdates()))}><RefreshCw size={13} />检查更新</button>}</div>
    ...
    <div className="settings-actions">
      {snapshot?.phase === 'available' && <button className="primary-button" disabled={busy} onClick={() => void run(async () => onUpdate(await window.norevinq.downloadUpdate()))}><Download size={13} />下载 {snapshot.availableVersion}</button>}
      {snapshot?.phase === 'downloaded' && <button className="primary-button" disabled={busy} onClick={() => void run(() => window.norevinq.installUpdate())}><RefreshCw size={13} />重启并安装</button>}
    </div>
    ...
    <div className="settings-actions"><button disabled={busy} onClick={() => void run(async () => {
      const result = await window.norevinq.exportDiagnostics()
      if (result.exported) setExportedFile(`${result.fileName ?? '诊断包'} · ${formatBytes(result.bytes)}`)
      onDiagnostics(await window.norevinq.getDiagnosticsState())
...
function ProviderTab({ providers, account, apiKey, setApiKey, busy, run, onUpdated, onAccount }: {
  ...
  const accountDetail = account?.account?.type === 'chatgpt'
    ...
    : account?.account?.type === 'apiKey'
      ? '凭据由 app-server 持久化；Norevinq 无法读回'
      : account?.account?.type === 'amazonBedrock'
  ...
  return <div className="settings-section">
    <div className="section-heading"><div><h3>OpenAI</h3><p>在 Norevinq 中管理 OpenAI 或 ChatGPT 认证状态。</p></div><button disabled={busy} onClick={() => void run(async () => onAccount(await window.norevinq.refreshOpenAiAccount()))}><RefreshCw size={13} />刷新</button></div>
    <div className="provider-state">
    ...
    {account?.pendingLogin ? <div className="provider-warning account-login-pending"><strong>{account.pendingLogin.type === 'deviceCode' ? '设备码登录等待完成' : '浏览器登录等待完成'}</strong>{account.pendingLogin.userCode && <code>{account.pendingLogin.userCode}</code>}<span>{account.pendingLogin.verificationUrl}</span><div className="settings-actions"><button disabled={busy} onClick={() => void run(async () => onAccount(await window.norevinq.cancelPendingChatGptLogin()))}>取消</button><button className="primary-button" disabled={busy} onClick={() => void run(async () => onAccount(await window.norevinq.openPendingChatGptLogin()))}><ExternalLink size={12} />重新打开</button></div></div> : account?.status === 'authenticated' ? <div className="settings-actions"><button disabled={busy} onClick={() => void run(async () => onAccount(await window.norevinq.logoutOpenAiAccount()))}>退出登录</button>{account.account?.type === 'chatgpt' && <button disabled={busy} onClick={() => void run(async () => onAccount(await window.norevinq.refreshOpenAiAccount({ refreshToken: true })))}><RefreshCw size={12} />刷新令牌</button>}</div> : <>
      <label className="credential-field"><span>OpenAI API Key</span><input type="password" autoComplete="new-password" spellCheck={false} value={openAiApiKey} onChange={(event) => setOpenAiApiKey(event.target.value)} placeholder="直接交给 app-server；Norevinq 不提供读回接口" /></label>
      <div className="settings-actions account-login-actions">
        <button disabled={busy} onClick={() => void run(async () => onAccount(await window.norevinq.startChatGptDeviceCodeLogin()))}>设备码登录</button>
        <button disabled={busy} onClick={() => void run(async () => onAccount(await window.norevinq.startChatGptBrowserLogin()))}><ExternalLink size={12} />浏览器登录</button>
        <button className="primary-button" disabled={busy || openAiApiKey.trim().length < 16} onClick={() => void run(async () => {
          const snapshot = await window.norevinq.loginOpenAiApiKey({ apiKey: openAiApiKey })
          setOpenAiApiKey('')
          onAccount(snapshot)
        })}>使用 API Key</button>
      </div>
    ...
    <div className="section-heading"><div><h3>DeepSeek Responses</h3><p>作为 Norevinq 的一级模型提供商，通过 Responses API 工作。</p></div></div>
    <div className="provider-state">
      <div className="provider-icon"><KeyRound size={17} /></div>
    ...
    <div className="settings-actions">
      {status?.credentialSource === 'os-vault' && <button onClick={() => void run(async () => onUpdated(await window.norevinq.deleteDeepSeekCredential()))} disabled={busy}>删除已保存密钥</button>}
      <button className="primary-button" onClick={() => void run(async () => {
        const result = await window.norevinq.saveDeepSeekCredential({ apiKey })
        setApiKey('')
        onUpdated(result)
...
function McpTab({ project, threadId, snapshot, busy, run }: {
  ...
  return <div className="settings-section">
    <div className="section-heading"><div><h3>MCP 服务器</h3><p>状态、OAuth、资源和直接工具诊断均由 app-server 提供。</p></div><button disabled={busy} onClick={() => void run(() => window.norevinq.reloadMcpServers({ projectId: project.id }))}><RefreshCw size={13} />重载</button></div>
    {snapshot?.loading ? <Loading /> : snapshot?.mcpServers.length === 0 ? <EmptySettings title="未配置 MCP" detail="在 Norevinq 智能体配置中添加 MCP 服务器后重载。" /> : <div className="integration-grid">
      ...
      {server && <div className="integration-detail">
        <div className="integration-title"><div><strong>{server.title ?? server.name}</strong><small>{server.version ?? '版本未报告'} · {server.authStatus}</small></div>{server.authStatus === 'notLoggedIn' && <button disabled={busy} onClick={() => void run(async () => {
          const result = await window.norevinq.startMcpOAuth({ projectId: project.id, name: server.name, ...(threadId ? { threadId } : {}) })
        ...
        {server.tools.length === 0 ? <p className="integration-empty">此服务器没有公开工具。</p> : <>
          ...
          <button className="primary-button inline" disabled={busy || !threadId || !tool || !confirmed} onClick={() => void run(async () => {
            const result = await window.norevinq.callMcpTool({
        ...
        {server.resources.length === 0 ? <p className="integration-empty">此服务器没有静态资源。</p> : <div className="resource-list">{server.resources.map((resource) => <button key={resource.uri} disabled={busy} title={resource.uri} onClick={() => void run(async () => {
          setResourceResult(await window.norevinq.readMcpResource({ projectId: project.id, name: server.name, uri: resource.uri, ...(threadId ? { threadId } : {}) }))
...
function SkillsTab({ project, snapshot, busy, run }: {
  ...
  return <div className="settings-section">
    <div className="section-heading"><div><h3>技能</h3><p>app-server 发现的真实技能；文件变化会自动刷新。</p></div><button disabled={busy} onClick={() => void run(() => window.norevinq.refreshIntegrations())}><RefreshCw size={13} />刷新</button></div>
    <div className={`trust-banner ${snapshot?.trusted ? 'trusted' : ''}`}><ShieldCheck size={16} /><div><strong>{snapshot?.trusted ? '项目已信任' : '项目未信任'}</strong><p>信任后才可添加进程级外部技能根目录；项目技能仍按兼容规则展示。</p></div><button disabled={busy} onClick={() => void run(() => window.norevinq.setProjectTrust({ projectId: project.id, trusted: !snapshot?.trusted }))}>{snapshot?.trusted ? '撤销信任' : '信任项目'}</button></div>
    <div className="extra-roots"><div><strong>额外技能目录</strong><small>只在当前 app-server 进程有效，不写入项目。</small></div><button disabled={busy || !snapshot?.trusted} onClick={() => void run(() => window.norevinq.chooseExtraSkillRoot({ projectId: project.id }))}><Plus size={12} />添加</button></div>
    {snapshot?.extraSkillRoots.map((root) => <div className="root-row" key={root}><code>{root}</code><button disabled={busy} onClick={() => void run(() => window.norevinq.removeExtraSkillRoot({ projectId: project.id, path: root }))}><Trash2 size={12} /></button></div>)}
    <div className="skill-list">{snapshot?.skills.map((skill) => <article key={skill.path}>
      ...
      <button className={skill.enabled ? 'enabled' : ''} disabled={busy} onClick={() => void run(() => window.norevinq.setSkillEnabled({ projectId: project.id, path: skill.path, enabled: !skill.enabled }))}>{skill.enabled ? <><Check size={12} />已启用</> : '已停用'}</button>
    </article>)}</div>
...
function ConfigTab({ project, snapshot, busy, run }: {
  ...
  const write = (key: SafeConfigKey, value: string): Promise<void> => run(() => window.norevinq.writeSafeConfig({ projectId: project.id, key, value }))
  return <div className="settings-section">
    <div className="section-heading"><div><h3>有效配置</h3><p>结构化写入用户 config.toml；托管要求和覆盖状态保持可见。</p></div><button disabled={busy} onClick={() => void run(() => window.norevinq.refreshIntegrations())}><RefreshCw size={13} />刷新</button></div>
    ...
    <h4>项目指令</h4>
    {snapshot?.instructions.length ? snapshot.instructions.map((instruction) => <details className="config-layer" key={instruction.path}><summary><FileText size={12} />{instruction.path}<span>{instruction.bytes} bytes</span></summary><pre>{instruction.preview}</pre></details>) : <p className="integration-empty">项目根目录没有 AGENTS.md 或 AGENTS.override.md；Norevinq 仍会按兼容层级规则寻找指令。</p>}
    <h4>权限配置</h4>
