...
export function BrowserWorkbench({ snapshot, width, setWidth, close, onError }: {
  ...
  useEffect(() => {
    void window.norevinq.openBrowser({}).catch((reason: unknown) => onError(errorMessage(reason)))
    const element = slot.current
    if (!element) return
    const update = (): void => {
      const bounds = element.getBoundingClientRect()
      if (bounds.width >= 100 && bounds.height >= 80) {
        void window.norevinq.setBrowserBounds({ x: bounds.x, y: bounds.y, width: bounds.width, height: bounds.height })
  ...
  async function dismiss(): Promise<void> {
    await run(() => window.norevinq.closeBrowser())
    close()
  }
  ...
  return <section className="browser-workbench" aria-label="本地网页预览">
    ...
    <header className="browser-toolbar">
      <button disabled={!snapshot?.canGoBack} onClick={() => void run(() => window.norevinq.goBackBrowser())} aria-label="后退"><ArrowLeft size={14} /></button>
      <button disabled={!snapshot?.canGoForward} onClick={() => void run(() => window.norevinq.goForwardBrowser())} aria-label="前进"><ArrowRight size={14} /></button>
      <button onClick={() => void run(() => snapshot?.loading ? window.norevinq.stopBrowser() : window.norevinq.reloadBrowser())} aria-label={snapshot?.loading ? '停止加载' : '刷新网页'}>{snapshot?.loading ? <Square size={11} fill="currentColor" /> : <RefreshCw size={13} />}</button>
      <form onSubmit={(event) => { event.preventDefault(); void run(() => window.norevinq.navigateBrowser({ url: address })) }}><input aria-label="本地预览地址" value={address} onChange={(event) => setAddress(event.target.value)} spellCheck={false} /><button type="submit" disabled={!address.trim()}>打开</button></form>
      <button disabled={!snapshot?.url} onClick={() => { if (snapshot?.url && window.confirm('在系统浏览器中打开当前地址？')) void run(() => window.norevinq.openBrowserExternal({ url: snapshot.url ?? '', confirmed: true })) }} aria-label="在系统浏览器打开"><ExternalLink size={13} /></button>
    ...
    <section className="browser-console" aria-label="网页控制台">
      <header><strong>控制台</strong><span>{snapshot?.logs.length ?? 0} 条</span><button disabled={!snapshot?.logs.length} onClick={() => void run(() => window.norevinq.clearBrowserLogs())}><Trash2 size={11} />清除</button></header>
