...
function RunInbox({ snapshot, busy, run }: {
  ...
  return <div className="settings-section"><div className="section-heading"><div><h3>运行收件箱</h3><p>需要关注的成功、失败、取消和跳过运行会标记未读。</p></div><button disabled={busy || !snapshot?.unreadRuns} onClick={() => void run(() => window.norevinq.markScheduledRunsRead({}))}><Check size={13} />全部已读</button></div>{runs.length === 0 ? <Empty title="没有运行记录" detail="计划任务运行后，结果和错误会显示在这里。" /> : <div className="scheduled-runs">{runs.map((item) => <article className={item.unread ? 'unread' : ''} key={item.id}><i className={item.status} /><div><strong>{item.taskName}</strong><span>{item.projectName} · 第 {item.attempt} 次 · {formatTime(item.scheduledFor)}</span><p>{item.summary ?? item.error ?? scheduledStatus(item.status)}</p></div><em>{scheduledStatus(item.status)}</em>{item.status === 'running' && <button disabled={busy} onClick={() => void run(() => window.norevinq.cancelScheduledRun({ runId: item.id }))}>取消</button>}{item.unread && <button disabled={busy} onClick={() => void run(() => window.norevinq.markScheduledRunsRead({ runIds: [item.id] }))}>已读</button>}</article>)}</div>}</div>
}
...
function TaskList({ snapshot, busy, edit, run }: {
  ...
  return <div className="settings-section"><div className="section-heading"><div><h3>计划任务</h3><p>本地项目任务仅在电脑开机且 Norevinq 运行时调度。</p></div></div>{tasks.length === 0 ? <Empty title="没有计划任务" detail="创建任务后可选择 Local 或隔离 worktree，并用 RFC 5545 RRULE 设置频率。" /> : <div className="scheduled-task-list">{tasks.map((task) => <article key={task.id}><header><div><strong>{task.name}</strong><span>{describeRule(task.rrule)} · {task.timezone}</span></div><i className={task.status}>{task.status === 'active' ? '运行中' : '已暂停'}</i></header><p>{task.prompt}</p><div className="task-chips"><span>{task.projectIds.length} 个项目</span><span>{task.executionTarget}</span><span>{task.sandbox}</span><span>下次：{task.nextRunAt ? formatTime(task.nextRunAt) : '—'}</span></div><footer><button disabled={busy} onClick={() => edit(task)}>编辑</button><button disabled={busy} onClick={() => void run(() => window.norevinq.runScheduledTaskNow({ taskId: task.id }))}><Play size={11} />立即运行</button><button disabled={busy} onClick={() => void run(() => window.norevinq.setScheduledTaskPaused({ taskId: task.id, paused: task.status === 'active' }))}>{task.status === 'active' ? <><Pause size={11} />暂停</> : <><RotateCcw size={11} />恢复</>}</button><button className="danger-button" disabled={busy} onClick={() => { if (window.confirm('删除计划任务？已有运行历史会保留。')) void run(() => window.norevinq.deleteScheduledTask({ taskId: task.id })) }}><Trash2 size={11} />删除</button></footer></article>)}</div>}</div>
}
