...
const enabled = process.env.ASTER_RUN_SECURITY_DEEPSEEK_ONLINE === '1'
  && Boolean(process.env.DEEPSEEK_API_KEY?.trim())
  && Boolean(process.env.ASTER_SECURITY_PYTHON?.trim())
const roots: string[] = []
const selectedModel = process.env.ASTER_SECURITY_DEEPSEEK_MODEL === 'deepseek-v4-flash'
  ? 'deepseek-v4-flash'
  : 'deepseek-v4-pro'
const preserveArtifacts = process.env.ASTER_KEEP_SECURITY_FIXTURE === '1'
...
describe.skipIf(!enabled)('Codex Security with DeepSeek Responses', () => {
  it('completes and seals a real repository scan without OpenAI credentials', async () => {
    const root = mkdtempSync(join(tmpdir(), 'aster-security-deepseek-online-'))
    roots.push(root)
    ...
    mkdirSync(state)
    writeFileSync(join(repository, 'src', 'safe.ts'), "export const ASTER_SECURITY_FIXTURE = 'read-only fixture'\n")
    ...
    const pythonPath = process.env.ASTER_SECURITY_PYTHON?.trim() ?? ''
    const configuredCodexPath = process.env.ASTER_SECURITY_CODEX_CLI_PATH?.trim()
