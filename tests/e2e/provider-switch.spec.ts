...
test('switches a real task from DeepSeek to OpenAI and back through the model picker', async () => {
  ...
  const profile = mkdtempSync(join(tmpdir(), 'aster-provider-ui-'))
  ...
  const application = await electron.launch({
    ...
    env: { ...process.env, ASTER_AGENT_HOME: agentHome },
  })
  try {
    ...
    await window.getByLabel('任务输入').fill('Reply with exactly ASTER_PROVIDER_DEEPSEEK_START and do not use tools.')
    await window.getByRole('button', { name: '发送任务' }).click()
    await expect(window.locator('.activity-card.agentMessage')
      .filter({ hasText: 'ASTER_PROVIDER_DEEPSEEK_START' })).toBeVisible({ timeout: 90_000 })
    await expect(window.locator('.running-row')).toHaveCount(0, { timeout: 30_000 })

    await window.getByLabel('模型').selectOption('gpt-5.6-sol')
    await window.getByLabel('任务输入').fill('Reply with exactly ASTER_PROVIDER_OPENAI_OK and do not use tools.')
    await window.getByRole('button', { name: '发送任务' }).click()
    await expect(window.locator('.activity-card.agentMessage')
      .filter({ hasText: 'ASTER_PROVIDER_OPENAI_OK' })).toBeVisible({ timeout: 90_000 })
    await expect(window.locator('.running-row')).toHaveCount(0, { timeout: 30_000 })

    await window.getByLabel('模型').selectOption('deepseek-v4-pro')
    await window.getByLabel('任务输入').fill('Reply with exactly ASTER_PROVIDER_DEEPSEEK_RETURN_OK and do not use tools.')
    await window.getByRole('button', { name: '发送任务' }).click()
    await expect(window.locator('.activity-card.agentMessage')
      .filter({ hasText: 'ASTER_PROVIDER_DEEPSEEK_RETURN_OK' })).toBeVisible({ timeout: 90_000 })
    await expect(window.locator('.running-row')).toHaveCount(0, { timeout: 30_000 })
    await expect(window.locator('.activity-card.error')).toHaveCount(0)
    await window.screenshot({ path: 'test-results/aster-provider-switch.png' })
  } finally {
