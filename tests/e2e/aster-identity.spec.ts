...
test.skip(!deepSeekConfigured, 'DEEPSEEK_API_KEY is required for the real Aster identity test.')

test('DeepSeek identifies the desktop product as Aster in an ordinary reply', async () => {
  test.setTimeout(120_000)
  const profile = mkdtempSync(join(tmpdir(), 'aster-identity-e2e-'))
  ...
  const application = await electron.launch({
    ...
    env: { ...process.env, ASTER_AGENT_HOME: join(profile, 'agent-home') },
  })
  try {
    ...
    await expect(answer).toContainText(/Aster(?: Code)?/iu)
