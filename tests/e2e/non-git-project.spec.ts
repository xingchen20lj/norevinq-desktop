...
test('opens a plain non-Git project without a worktree fatal error', async () => {
  const profile = mkdtempSync(join(tmpdir(), 'norevinq-non-git-e2e-'))
  ...
  try {
    ...
    await expect(panel).toContainText('普通 Norevinq 任务仍可使用')
    await expect(panel.getByRole('button', { name: '创建', exact: true })).toHaveCount(0)
    await window.screenshot({ path: 'test-results/norevinq-non-git-project.png' })
  } finally {
