...
| 顶栏 | 项目/工作树上下文、任务标题、Handoff、Git/终端/预览入口 | 部分实现：真实重命名、分叉、压缩、归档/恢复/删除、分支、变更计数、Git、终端和文件预览入口；Handoff 待开发 |
...
| 审批 | 明确动作、理由、影响范围、允许一次/会话/拒绝 | 部分实现：真实文件审批与 MCP/用户输入挂起面板已验证；命令拒绝和 permissions profile E2E 待补 |
| 设置 | 外观、模型/提供商、权限、工作树、MCP、技能、插件、快捷键、日志 | 部分实现：提供商/MCP/技能/配置/应用更新/本地诊断导出、system/light/dark、固定核心快捷键与命令面板已验证；插件稳定 API、快捷键重映射和日志级别设置待补 |
