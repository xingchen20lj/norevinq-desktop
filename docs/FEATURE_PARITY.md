...
| 测试 | 单元、集成、E2E、桌面冒烟 | 部分实现 | 34 个测试文件 159 项及 V8 全局门槛；官方 Codex 0.147.0 无模型 account/thread/goal/permission-profile 集成、离线 Electron 生命周期/固定/深链接/更新/崩溃诊断/权限审批/账户、配置渠道与普通目录包、挂载 DMG E2E 通过；Windows 跨平台路径/工作树/进程/CRLF/mode 回归已补，远端 CI 复跑与目标真机、在线账户恢复待验证 |
