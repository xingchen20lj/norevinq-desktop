...
| 导航 | 项目、任务、计划任务、安全、设置导航 | 部分实现 | 项目/任务与四页设置工作台可用；计划任务和安全工作台待接入 |
...
| 审批 | MCP/应用工具审批 | 部分实现 | 直接工具调用显式确认、MCP elicitation 挂起/取消已测试；permissions/app 专属审批待补 |
...
| 设置 | 模型、推理、权限、外观 | 部分实现 | 四页工作台覆盖提供商、MCP、技能、审批/沙箱/Web Search；快捷键和完整权限 profile 待补 |
...
| 测试 | 单元、集成、E2E、桌面冒烟 | 部分实现 | 19 个测试文件 85 项；真实 Electron 覆盖 OpenAI、DeepSeek、AGENTS、MCP、审批、Git/diff/worktree 和 PTY 终端 |
