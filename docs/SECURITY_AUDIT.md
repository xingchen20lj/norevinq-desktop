...
审计日期：2026-08-11，生产依赖复审于 2026-08-13，开源发布复核于 2026-08-14，修复复验于 2026-08-15。范围为当前仓库生产源码、Electron 配置、IPC、文件/浏览器边界、子进程、凭据、生产依赖和发布流水线。

## 2026-08-15 修复复验结论

- 2026-08-14 sealed 扫描的 3 项中风险问题均已修复并加入回归：应用拒绝把选中的项目子目录扩大为父 Git 仓库；所有应用 Git 调用显式禁用 `core.fsmonitor`；Security SDK 扫描权限从主机根目录读取收紧为运行时最小读取、工作区只读和专用状态目录写入。
- 5 项低风险问题中，4 项已关闭：文本预览使用同一 `O_NOFOLLOW` 文件描述符完成身份校验和读取；目录枚举与未跟踪文件 diff 在加载前执行硬上限；单架构包只保留目标架构且包检查要求恰好一个运行时；GitHub 已删除安装包发布 workflow，CI 守门禁止重新上传桌面产物。
- 1 项低风险纵深防御问题被明确接受：如果受沙箱保护的主 Renderer 本身已被攻破，攻击代码仍可能伪造 Renderer 侧“已确认”字段。现有补偿控制包括 Renderer sandbox、context isolation、CSP、顶层主窗口 IPC 身份校验、结构化参数校验、危险文件类型拒绝和服务端操作约束。后续公开安装包前应将终端、直接 MCP、漏洞修复和外部打开迁移到主进程持有的原生用户确认能力；该残余风险不阻塞只发布源码的 0.1.0 预览版。
- 加固后的 `@openai/codex-security` 0.1.11 pnpm 补丁可从干净依赖安装重放；真实 SDK 本地 preflight 成功。权限配置的自动测试确认不存在 `:root`、工作区为只读、只有扫描状态目录可写。未把未执行的恶意仓库在线扫描伪称为动态验证。
- `pnpm verify:ci` 通过：214 项测试通过、1 项显式在线测试跳过；覆盖率 statements 81.33%、branches 70.30%、functions 86.40%、lines 88.24%。生产依赖审计为 0 个已知高危漏洞，离线 IPC 对抗和实际目录包启动测试均通过。
- 因此当前源码满足公开预览条件；项目仍不在 GitHub 分发未签名安装包。正式二进制分发继续要求独立签名、公证和目标平台复验。
...
- 后续扫描确认当时预览版有 3 项中等和 5 项低风险工程问题；其修复与接受状态见上方 2026-08-15 复验结论。该历史结论不再代表当前源码状态。
