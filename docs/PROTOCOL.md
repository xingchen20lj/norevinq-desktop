...
- `clientInfo.name` 固定为 `norevinq`；默认只启用 stable API。
...
1. 在 Electron `userData/agent-home` 创建私有 `0700` Norevinq 智能体目录；开发/E2E 仅允许用绝对 `NOREVINQ_AGENT_HOME` 显式覆盖。旧产品目录 `aster-code` 会在首次打包启动时安全迁移为 `norevinq`，其中的 `codex-home` 再迁移为 `agent-home`；旧 `ASTER_AGENT_HOME`/`ASTER_CODEX_HOME` 只作为兼容别名读取。
2. 发现并探测 Codex 二进制。
...
Norevinq 不读取或写入官方客户端默认的 `~/.codex`。登录、thread、用户级 MCP/技能设置保存在 Norevinq 私有 `agent-home`；启动上游进程时仅通过兼容环境变量 `CODEX_HOME` 指向该目录。项目仓库中的 `.codex` 与 `AGENTS.md` 仍按上游公开层级规则生效。首次运行需要在 Norevinq 内单独登录，避免退出登录、任务列表或技能切换影响官方 Codex 桌面客户端。

面向普通用户的产品身份为 Norevinq。新建、恢复和分叉任务均传入稳定的 `developerInstructions`，要求日常回答自称 Norevinq；当用户询问架构、许可、模型或诊断时，仍必须如实说明 OpenAI 开源 Codex app-server 与实际模型提供商。技术协议名和第三方归属不会被重命名或隐藏。
...
- 2026-08-10 Playwright Electron E2E 在临时真实项目中创建 thread/turn，使用现有 ChatGPT 登录完成在线 SSE 活动，并验证最终消息 `NOREVINQ_RUNTIME_OK`、完成状态、深浅主题和 960×640 布局。
- 同一隔离 E2E 以 read-only 沙箱真实触发 fileChange 反向审批；用户允许后 `apply_patch` 创建文件并精确断言 `NOREVINQ_APPROVAL_OK\n`。随后真实执行 `sleep 8`、在运行中 steer 至 `NOREVINQ_STEER_OK`，并中断另一条 `sleep 20`，确认不产生被禁止的最终回复。
