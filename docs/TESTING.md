...
2. 集成测试使用真实临时 SQLite、Git 仓库和工作树；数据库覆盖 v7→v8 固定元数据迁移、排序和 20 项任务补读上限。任务生命周期还会直接启动随依赖固定的官方 `@openai/codex` 0.147.0 app-server，在隔离 `CODEX_HOME` 中验证命名、读取、搜索请求、分叉、归档、恢复、删除及长期目标 set/get/clear，不调用模型或消耗在线额度。
