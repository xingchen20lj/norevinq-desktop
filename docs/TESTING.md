...
2. 集成测试使用真实临时 SQLite、Git 仓库和工作树；数据库覆盖 v7→v8 固定元数据迁移、排序、20 项任务补读上限和项目–任务深链接关联。任务生命周期还会直接启动随依赖固定的官方 `@openai/codex` 0.147.0 app-server，在隔离 `CODEX_HOME` 中验证命名、读取、搜索请求、分叉、归档、恢复、删除、长期目标 set/get/clear 与 `permissionProfile/list`，不调用模型或消耗在线额度。
...
5. Electron E2E 使用临时真实仓库，验证沙箱 Renderer、在线 Codex、DeepSeek（存在密钥时）、审批允许/拒绝与网络/路径逐项授权、Git/diff/worktree、终端、文件预览、本地网页、计划任务、单实例深链接、更新设置和应用重启恢复；深链接只允许 UUID 目标并在主进程重新校验 SQLite 关联。
