...
Norevinq 使用分层验证，避免以静态界面或模拟数据替代真实闭环。
...
- `NOREVINQ_UPDATE_URL='https://updates.invalid/norevinq/' pnpm package:update`：仅用于本地生成元数据测试，验证 app-update/latest/blockmap/SHA-512；`.invalid` 构建不可发布。
...
- `pnpm exec playwright test tests/e2e/non-git-project.spec.ts`：打开真实普通文件夹，验证首页无 `worktree:list` Git fatal，工作树显示 Local-only 引导且 Norevinq 任务入口仍可用。
- `DEEPSEEK_API_KEY=... pnpm exec playwright test tests/e2e/norevinq-identity.spec.ts`：用 DeepSeek V4 Flash 发起最小真实任务，验证普通回答使用 Norevinq 产品身份且不出现 Codex；无 Key 时自动跳过。
...
- 2026-08-11 一次性显式线上回归：Norevinq Electron 主进程真实创建并回读私有 Draft PR #1；验证 `~/bin/gh` 发现、真实 push、owner/head/base/URL 和在线幂等。该外部写入测试不进入常规 CI，避免在重复运行中修改用户仓库。
- `NOREVINQ_TEST_LIVE_AUTH=1 pnpm exec vitest run tests/integration/codexThreadLifecycle.test.ts`：在隔离 `CODEX_HOME` 中额外向官方认证服务发起设备码登录并立即取消；不调用模型、不完成用户授权，常规 CI 不依赖该网络检查。
