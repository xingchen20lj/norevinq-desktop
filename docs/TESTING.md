...
2. 集成测试使用真实临时 SQLite、Git 仓库和工作树；Git 覆盖 staged/unstaged/untracked/rename 整文件恢复、已有用户 stash 保留及目标占用失败关闭。数据库覆盖 v7→v8 固定元数据迁移、排序、20 项任务补读上限和项目–任务深链接关联。任务生命周期还会直接启动随依赖固定的官方 `@openai/codex` 0.147.0 app-server，在隔离 `CODEX_HOME` 中验证 API Key login/read/logout、ChatGPT 浏览器 start/cancel、命名、读取、搜索、分叉、归档、恢复、删除、长期目标和 `permissionProfile/list`；默认不调用模型或联网。
