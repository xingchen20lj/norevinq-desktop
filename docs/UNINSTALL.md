...
Aster 不安装 LaunchAgent、LaunchDaemon、内核扩展或常驻后台服务；计划任务仅在 Aster Code 主程序运行时执行。
...
if [ -x "$LSREGISTER" ] && [ -d "/Applications/Aster Code.app" ]; then
  "$LSREGISTER" -u "/Applications/Aster Code.app"
fi
```
...
若应用安装在当前用户的“应用程序”目录，也删除 `~/Applications/Aster Code.app`。确认工作树和扫描报告都已备份后，再清空废纸篓。
