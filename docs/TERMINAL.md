...
- Norevinq 直接使用上述 PTY 协议，不解析 Codex TUI 文本，也不引入 `node-pty` 原生模块。
...
- 清屏同时发送 Form Feed 给 shell 并清除 Norevinq 缓冲；关闭运行中会话会先请求 terminate。
...
- E2E 将当前终端输出显式共享给真实 Codex，收到 `NOREVINQ_TERMINAL_CONTEXT_OK`；随后验证清屏、terminate、退出状态和关闭会话。
