...
阶段 17：完整自动测试（代码与离线回归完成，在线复验受账户使用量外部限制）。

## 当前任务

- 完成阶段 17 文档与提交。
- 进入阶段 18，执行依赖审计、静态安全审计、Codex Security 可用范围验证和加固。
...
1. 提交阶段 17 的测试与 CI 检查点。
2. 运行依赖漏洞/许可证检查与仓库安全扫描，验证发现后修复并重新回归。
3. 审计 IPC、子进程、路径、浏览器隔离、凭据和日志边界。
4. 账户使用量恢复后重新执行在线 Electron E2E 与真实应用重启恢复场景。
...
离线工程检查无失败：`verify:ci` 的 25 个测试文件 107 项、覆盖率门槛、类型、规范和构建均通过。在线 Electron E2E 因账户使用量耗尽在计划任务真实 turn 返回外部错误；不是代码断言失败，必须在额度恢复后复验。
