...
- 建立全局快捷键、命令面板和键盘导航。
- 完成窗口尺寸/主题恢复、系统主题实时同步和关键面板焦点管理。
- 优化工作台路由、空/加载/错误状态以及 960×640 响应式布局。
- 进行深浅主题、长文本、大列表和无障碍视觉回归。
...
暂无。最近一次结果：24 个单元/集成测试文件共 104 项通过；Electron E2E 1 项约 2 分钟通过，真实覆盖本地浏览器、文件文本/图片产物、计划任务、Security SDK preflight、AGENTS.md、MCP/技能/配置、app-server PTY、终端输出→Codex，以及既有全功能回归；类型、规范、脚本语法和生产构建通过。
...
- 命令面板和快捷键是否需要用户级完全自定义，还是先提供冲突检测后的固定核心映射。
