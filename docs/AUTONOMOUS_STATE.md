...
- 调查 Electron WebContentsView 的隔离、导航和本地服务边界。
- 实现受限本地网页预览、地址导航、刷新、停止和外部打开。
- 捕获页面加载错误、控制台和本地开发服务状态，不暴露主应用权限。
- 建立浏览器预览 UI、自动测试和桌面回归。
...
1. 选择 Electron 隔离浏览上下文并建立 URL/导航策略。
2. 实现本地 HTTP 页面预览、刷新、停止、错误和控制台日志。
3. 建立浏览器预览 UI 与项目服务上下文。
4. 完成真实本地网页回归、视觉检查、文档与阶段提交。
...
暂无。最近一次结果：23 个单元/集成测试文件共 101 项通过；Electron E2E 1 项约 1.7 分钟通过，真实覆盖文件文本/图片产物、计划任务、Security SDK preflight、AGENTS.md、MCP/技能/配置、app-server PTY、终端输出→Codex，以及既有全功能回归；类型、规范、脚本语法和生产构建通过。
...
- 本地网页预览是否需要为每个项目保留独立持久 session，或默认使用内存临时 partition。
