...
- 审计功能一致性表中仍缺少的关键回归，并补单元/集成测试。
- 拆分单体 Electron E2E 为可诊断场景，增加重启恢复、崩溃、拒绝和错误状态。
- 建立覆盖率/测试报告、跨平台 CI 测试矩阵和稳定 fixture。
- 执行完整回归并修复发现的问题。
...
1. 生成测试/能力缺口清单并优先补 app-server 崩溃与重启恢复。
2. 拆分和增强 Electron E2E，加入错误/拒绝/持久化场景。
3. 建立 macOS/Windows CI 与覆盖率报告。
4. 执行完整测试循环并修复回归。
...
暂无。最近一次结果：24 个单元/集成测试文件共 104 项通过；Electron E2E 1 项约 2.2 分钟通过，真实覆盖命令/侧栏/主题/窗口持久化、本地浏览器、文件产物、计划任务、Security SDK preflight、AGENTS.md、MCP/技能/配置、PTY 终端及既有回归；类型、规范、脚本语法和生产构建通过。
...
- app-server 进程崩溃注入测试如何在不重复执行副作用 turn 的前提下验证 idle 自动恢复与 active 失败状态。
