...
- 建立受约束文件树、文本/代码读取和大文件预算。
- 实现图片、音频、视频和 PDF 的安全本地预览。
- 从智能体活动和工作区发现真实文件产物，并支持安全外部打开。
- 建立文件预览 UI、自动测试和桌面回归。
...
1. 设计文件树、目录遍历、符号链接和路径边界。
2. 实现有界文本/代码/媒体/PDF 元数据与预览协议。
3. 建立产物发现、预览和外部打开 UI。
4. 完成真实文件回归、视觉检查、文档与阶段提交。
...
暂无。最近一次结果：21 个单元/集成测试文件共 93 项通过；Electron E2E 1 项约 1.7 分钟通过，真实覆盖计划任务、Security SDK preflight、AGENTS.md、MCP/技能/配置、app-server PTY、终端输出→Codex，以及既有全功能回归；类型、规范、脚本语法和生产构建通过。
...
- 文件预览对超大文本、PDF 和媒体解码采用何种渐进式加载上限。
